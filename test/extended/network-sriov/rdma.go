...
var _ = Describe("[Area:Networking][Serial] SRIOV RDMA", func() {
	defer GinkgoRecover()
	...
	InNetworkAttachmentContext(func() {
		...
		It("should report correct rdma interface number", func() {
			...
			if len(resConfList.ResourceList) > 0 {
				for _, dev := range nicMatrix.NICs {
					...
					err := oc.AsAdmin().Run("create").
						...
						RDMATestDataFixture, dev.ResourceName)).Execute()
					Expect(err).NotTo(HaveOccurred())
				}
				By("Creating SRIOV device plugin config map")
				err := oc.AsAdmin().Run("create").
					Args("-f", fmt.Sprintf("%s/%s", RDMATestDataFixture, sriovDPConfigMap),
				...
				e2e.Skipf("Skipping, no SR-IOV RDMA capable NIC configured.")
			}
					...
					for _, dev := range nicMatrix.NICs {
						...
						fmt.Sprintf("%s/%s", RDMATestDataFixture, sriovDPConfigMap),
						"-n", "kube-system").Execute()
                                        Expect(err).NotTo(HaveOccurred())
			...
			for _, n := range resConfList.ResourceList {
				By("Creating SRIOV RDMA Test Pod")
				err := oc.AsAdmin().Run("create").
					Args("-f", fmt.Sprintf("%s/pod-%s-rdma.yaml",
					RDMATestDataFixture, n.ResourceName)).Execute()
				Expect(err).NotTo(HaveOccurred())

				By("Waiting for testpod become ready")
				err = wait.PollImmediate(e2e.Poll, 3*time.Minute, func() (bool, error) {
					err = CheckPodStatus(oc, fmt.Sprintf("testpod-%s-rdma", n.ResourceName))
					if err != nil {
						return false, nil
					}
					return true, nil
				...
				out, err := oc.AsAdmin().Run("exec").
					Args(fmt.Sprintf("testpod-%s-rdma", n.ResourceName),
